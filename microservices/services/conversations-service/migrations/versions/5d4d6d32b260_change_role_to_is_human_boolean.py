...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE messages SET is_human = CASE WHEN role = 'user' THEN true ELSE false END"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE messages SET role = CASE WHEN is_human = true THEN 'user' ELSE 'assistant' END"
    )
