...
class TestGetConversationMessages:
    ...
    async def test_get_messages_empty_conversation(self, client, db_session):
        ...
        conversation = Conversation(
            id=conversation_id, user_id=user_id, title="Empty Conversation"
        )
        db_session.add(conversation)
        await db_session.flush()

        response = await client.get(
            f"/api/users/{user_id}/conversations/{conversation_id}/messages"
        )
    ...
    async def test_get_messages_with_data(self, client, db_session):
        ...
        # Create conversation
        conversation = Conversation(
            id=conversation_id, user_id=user_id, title="Test Conversation"
        )
        db_session.add(conversation)
        await db_session.flush()
        ...
        assert (
            messages[1]["content"] == "I'd be happy to help with your career questions!"
        )
        assert messages[2]["is_human"] == True
        assert (
            messages[2]["content"]
            == "What skills should I focus on for software engineering?"
        )
    ...
    async def test_get_messages_wrong_user(self, client, db_session):
        ...
        conversation = Conversation(
            id=conversation_id, user_id=user1_id, title="User 1's Conversation"
        )
        db_session.add(conversation)
        await db_session.flush()
        ...
        response = await client.get(
            f"/api/users/{user2_id}/conversations/{conversation_id}/messages"
        )
    ...
    async def test_get_messages_message_isolation(self, client, db_session):
        ...
        conversation1 = Conversation(
            id=conversation1_id, user_id=user_id, title="Conversation 1"
        )
        conversation2 = Conversation(
            id=conversation2_id, user_id=user_id, title="Conversation 2"
        )
        ...
        response1 = await client.get(
            f"/api/users/{user_id}/conversations/{conversation1_id}/messages"
        )
        assert response1.status_code == 200
        data1 = response1.json()
        assert len(data1["messages"]) == 2
        assert all(
            msg["conversation_id"] == str(conversation1_id) for msg in data1["messages"]
        )

        # Test conversation 2 messages
        response2 = await client.get(
            f"/api/users/{user_id}/conversations/{conversation2_id}/messages"
        )
    ...
    async def test_get_messages_invalid_conversation_id(self, client):
        ...
        response = await client.get(
            f"/api/users/{user_id}/conversations/invalid-uuid/messages"
        )
    ...
    async def test_get_messages_response_structure(self, client, db_session):
        ...
        conversation = Conversation(
            id=conversation_id, user_id=user_id, title="Schema Test Conversation"
        )
        message = Message(
            id=uuid4(),
            conversation_id=conversation_id,
