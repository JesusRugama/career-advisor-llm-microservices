        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Error fetching conversation {conversation_id}: {response.status_code} - {response.text}"
                    )
                    return None

            except httpx.RequestError as e:
                print(
                    f"Request error when fetching conversation {conversation_id}: {e}"
                )
                return None
            except Exception as e:
                print(
                    f"Unexpected error when fetching conversation {conversation_id}: {e}"
                )
                return None
